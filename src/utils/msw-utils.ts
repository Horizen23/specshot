...
import type { FakerPlugin } from "../core/config-loader";
  ...
  const callbacks: TraverseSchemaCallbacks<string> = {
    ...
    onPrimitive: (schema, context) => {
      ...
      if (mockMode === "faker") {
        ...
        // 1. Explicit plugin selected in UI
        if (customFormat && customFormat.startsWith("plugin:")) {
          const pluginName = customFormat.replace("plugin:", "");
          const plugin = fakerPlugins.find((p) => p.name === pluginName);
          if (plugin) {
            try {
        ...
        // 3. Auto plugin match
          ...
          for (const plugin of fakerPlugins) {
            try {
